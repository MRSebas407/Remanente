    ...
    <div class="p-4 overflow-y-auto flex-1">
      <form (ngSubmit)="onSubmit()" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium mb-1 text-primary">Contraseña</label>
            <input type="text" [value]="'Cédula (' + document + ')'" disabled
              class="w-full px-3 py-2 rounded-lg border border-theme bg-secondary text-secondary text-sm" />
          </div>
        </div>
...
export class AdviserCreate implements OnInit {
